...
교육에서는 JB금융그룹의 사업 방향성과 연계된 AI Agent 서비스를 직접 기획·개발·검증·발표 자료화하는 흐름을 사용합니다.
...
설치 후에는 프로젝트 폴더에서 Claude Code CLI를 실행할 수 있어야 합니다.
...
Claude Code CLI에서 Harness 플러그인을 설치합니다.
...
| `/reload` 실행 | 새로 설치한 플러그인이 Claude Code 세션에 반영됨 |
| Harness 호출 가능 | `/harness` 또는 “하네스 구성해줘” 요청을 사용할 수 있음 |

> 포인트: 플러그인을 설치한 뒤에는 바로 작업하지 말고 `/reload`를 먼저 실행합니다. 새 기능이 현재 세션에 반영되어야 합니다.

---

### 6.4 기능 명세서와 MVP 제안서 만들기

이제 Claude Chat 또는 Claude Cowork에서 서비스 기획 산출물을 만듭니다.

여기서는 `Compliance AI - 준법자문가 AI Agent 서비스 개발`을 예시로 사용합니다. 아래 프롬프트를 그대로 복사해 사용합니다.
...
| 보안·개인정보·내부통제 고려가 있는가 | 금융 서비스 제안서에서 필수 검토 항목 |

> 포인트: 이 단계의 목적은 완벽한 문서가 아니라, Claude Code가 읽고 코드를 만들 수 있는 “충분히 구체적인 기준 자료”를 만드는 것입니다.

---

### 6.5 가장 간단한 MVP 소스 코드 만들기

기능 명세서와 MVP 제안서를 프로젝트 폴더에 넣었다면, Claude Code IDE에서 폴더를 열고 가장 작은 형태의 소스 코드를 만듭니다.

아래 프롬프트를 Claude Code IDE에 입력합니다.

```text
기능 명세서와 MVP 제안서를 읽고 가장 간단한 형태의 소스 코드를 개발해줘.
```

이 요청에서 중요한 것은 “완성도 높은 상용 서비스”가 아니라 “핵심 흐름이 보이는 MVP”입니다.

예를 들어 Compliance AI라면 아래 흐름이 최소한 보이면 좋습니다.
...
이제 Claude Code CLI에서 Harness를 실행합니다.

먼저 프로젝트 폴더 안에 아래 자료가 있는지 확인합니다.
...
```text
프로젝트 안에 기능 명세서, MVP 제안서, 소스 코드를 읽고 AI Agent 서비스를 검증하고 고도화하는 하네스를 구성해줘.
```
...
| 검증 루프가 있는가 | 생성만 하고 끝나지 않고 점검과 개선으로 이어져야 함 |
| 기능 명세서와 코드가 연결되는가 | 문서와 구현이 따로 놀지 않아야 함 |
| 금융 리스크 검토가 포함되는가 | 개인정보, 규제, 내부통제, 환각 위험을 봐야 함 |
| 출력물이 사람이 읽기 쉬운가 | 교육 수강생이 결과를 이해하고 수정할 수 있어야 함 |

> 포인트: 좋은 Harness는 “더 많은 에이전트”가 아니라, **필요한 검증 관점을 빠짐없이 가진 작은 팀**입니다.

---

### 6.7 검증 및 고도화 Skills 생성하기

마지막으로 이 프로젝트에서 반복 사용할 검증·고도화 Skills를 정리합니다.

원본 실습 흐름에서는 이 단계를 다음처럼 표현합니다.

> 검증 및 고도화 skills 생성!

실제 교육에서는 아래처럼 구체적으로 요청하면 수강생이 결과물을 이해하기 쉽습니다.
...
| 반복 실행 가능성이 있는가 | 다음 프로젝트에서도 skill을 재사용할 수 있음 |
...
이 단계에서는 PPT를 바로 만들지 않습니다. 먼저 Codex가 참고할 기준 문서를 만들고, 그 다음 PPT를 생성하고, 렌더 이미지로 검수하고, 실제 PowerPoint 화면에서 최종 확인합니다.

진행 순서는 다음과 같습니다.

1. Codex로 발표 자료 제작 폴더 준비
2. `getdesign.md`에서 `DESIGN.md` 복사해 붙여넣기
...
Codex에서 프로젝트 폴더를 열고, 아래 프롬프트를 입력합니다.

이 단계의 목적은 PPT 파일을 바로 만드는 것이 아니라, Codex가 일할 수 있는 작업 환경을 만드는 것입니다.
...
다음으로 `getdesign.md`에서 `DESIGN.md` 내용을 복사해 `brand/DESIGN.md`에 붙여넣습니다.

원본 실습 순서는 다음과 같습니다.

```text
getdesign.md에서 DESIGN.md Copy 해서 붙여넣기
```

이 단계가 필요한 이유는 디자인 기준을 AI가 매번 새로 상상하지 않게 하기 위해서입니다. `DESIGN.md`에는 최소한 아래 내용이 들어가면 좋습니다.
...
기준 문서와 디자인 기준을 확인한 뒤에야 1차 PPT를 만듭니다.

Codex에서 아래 프롬프트를 입력합니다.
...
| 문구 | 이유 |
|---|---|
| `output/presentation.pptx 생성` | 파일 위치를 명확히 해야 나중에 검수 가능 |
| `편집 가능한 객체로 유지` | PPT에서 사람이 직접 수정해야 함 |
| `핵심 텍스트는 이미지 안에 넣지 말 것` | 이미지화된 글자는 수정과 검수가 어려움 |
| `렌더 검수는 다음 단계` | 생성과 검수를 분리해야 문제 원인을 찾기 쉬움 |

> 포인트: PPT는 예쁘기만 하면 안 됩니다. 사람이 수정할 수 있고, 검수할 수 있고, 평가 기준과 연결되어야 합니다.

---

### 7.4 렌더 이미지와 montage로 1차 검수하기

PPT 초안이 생성되면 바로 최종본으로 쓰지 않습니다. 먼저 슬라이드별 PNG와 전체 montage를 만들어 눈으로 볼 수 있게 검수합니다.
...
| 출처 누락 | 외부 규정이나 기사 인용 출처 없음 | 출처 슬라이드 또는 각주 추가 |
| 민감정보 오해 | 실제 내부자료처럼 보이는 표현 | 교육용 가상 예시임을 명시 |

> 포인트: 검수 단계에서는 바로 고치지 않습니다. 먼저 문제를 목록화해야 수정 범위가 분명해집니다.

---

### 7.5 PPT 수정본 만들기

검수 결과에서 수정할 항목이 나오면, 원본을 덮어쓰지 말고 새 버전으로 만듭니다.
...
> 포인트: 실무에서는 “최종_진짜최종.pptx”가 늘어나는 것을 막기 위해 버전명과 검수 결과를 같이 남기는 습관이 중요합니다.

---

### 7.6 Computer Use로 최종 확인하기

렌더 검수까지 마쳤다면 마지막으로 실제 PowerPoint에서 열어봅니다. 렌더 이미지만으로는 실제 앱에서만 보이는 폰트 대체, 슬라이드 쇼 전환, 편집 화면 문제를 놓칠 수 있습니다.
...
| 상황 | 권장 방식 |
|---|---|
| 슬라이드 전체 톤만 보고 싶음 | `review-montage-v2.png` 먼저 확인 |
| 글자 잘림과 이미지 crop 확인 | 렌더 이미지와 PowerPoint 화면 모두 확인 |
| 발표 모드 전환 확인 | 실제 PowerPoint에서 슬라이드 쇼 실행 |
| 계정, 결제, 보안 설정 화면 등장 | AI가 조작하지 않게 하고 사람이 직접 처리 |
| 개인 파일 접근 가능성이 있음 | “이 파일만 열 것”처럼 범위를 좁게 지정 |

> 포인트: Computer Use는 강력하지만, 범위를 좁게 제한해야 합니다. 이 실습에서는 최종 PPT 파일만 열고 확인합니다.

---

### 7.7 최종 파일과 검수 결과 정리하기

마지막으로 프로젝트를 다음 제안서 제작에도 재사용할 수 있게 정리합니다.
...
| 버전 파일을 덮어씀 | 수정 전후 비교가 어려움 | `presentation-v2.pptx`처럼 버전 분리 |
...
네. 다만 핵심 기능, 사용자, 사용 상황, 시스템 흐름은 있어야 합니다. 문서가 너무 추상적이면 Claude Code가 코드를 만들 때 임의로 해석하게 됩니다.
...
이 흐름을 한 번 만들어두면 다음 AI Agent 서비스 제안에서도 주제와 데이터만 바꿔 반복 사용할 수 있습니다.
